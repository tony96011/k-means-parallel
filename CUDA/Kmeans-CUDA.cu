...
__global__ void assign_clusters(Point *points, Point *centr,int K, int N, double *distances){
    ...
    for(int j=0; j<K ; j++){
        distances[point_id*N+j] = euclid(points[point_id], centr[j]);
    }
    int index = 0;
    for(int i = 1; i < K; i++)
    {
        if(distances[point_id*N+i] < distances[index])
            index = i;
    }
...
void kmeans_CUDA(int N, int K, int* data_points, int** data_point_cluster, float** centroids, int* num_iterations){
    ...
    cudaMalloc((void**)&d_centr, N * sizeof(Point));
    cudaMemcpy(d_centr, centr, N * sizeof(Point), cudaMemcpyHostToDevice); 
    ...
    double d_distances[K];
    cudaMalloc((void**)&d_distances, K * sizeof(double));
